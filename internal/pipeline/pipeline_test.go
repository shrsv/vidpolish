...
func TestStageReporterZeroOverallShowsEstimating(t *testing.T) {
	...
	r := opts.newStageReporter(time.Now(), 0) // split: bounds 0.00-0.08
	r.report(0)
...
func TestStageReporterComputesOverallPercentAndETA(t *testing.T) {
	...
	r := opts.newStageReporter(start, 1)       // denoise: bounds 0.08-0.75
...
func TestStageReporterOverallProgressesMonotonically(t *testing.T) {
	...
	start := time.Now()
	r := opts.newStageReporter(start, 1) // denoise
	r.report(0.1)
	r.report(0.5)
	r.report(0.9)
	if len(messages) != 3 {
...
func TestStageReporterFallsBackToStdoutWithoutLogCallback(t *testing.T) {
	...
	opts := Options{}
	r := opts.newStageReporter(time.Now(), 2)
	r.report(0.25)
}
